...
Transporter/Portal Magenta (`240, 130, 230`), Fackel Kerzenlicht (`255, 226,
160` – heller und gelber als Brüchig-Bernstein, blasser als Schlüssel-Gold:
Licht, kein Ding; M66), Partner-BALL Rot (`255, 96,
...
| `.ed-badge` + `#edCheckSheet` | Beweis-Badges sind KNÖPFE (M71): Tippen öffnet ein MODAL (M73) – Schirm `rgba(5, 7, 15, 0.72)` wie `#interstitial` über allem im Editor, darin eine OPAKE Karte (`.ed-sheet-card`, `--bg-panel`) unten in Daumennähe, Tap daneben schließt. Inhalt: Titel im Zustandston (✓ `--accent`, ⚠/✗ `--warning`), ein Satzpaar „was geprüft wird / was ein Rot bedeutet", das Detail in Klartext (`--warning`) und „👁 Zeigen". Die gezeigte Zelle bekommt einen bernsteinfarbenen Rahmen, der bleibt, bis man das Feld anfasst. WARUM MODAL: Als bodennahe Karte lag sie durchsichtig über dem Eigenschaften-Griff – zwei Ebenen Text übereinander, beides unlesbar |
| `#mpNetStatus` / `#mpReconnectBtn` / `#mpNetDebug` | Lobby-Diagnose (M70): Die kurze Warnzeile (`--warning`) erscheint NUR, wenn kein Vermittler erreichbar ist oder nach 35 s niemand kam – eine ruhige Lobby bleibt ruhig. „🔄 Neu verbinden" steht dagegen die ganze Wartezeit da. `#mpNetDebug` ist die Monospace-Tafel für `?netdebug`/Debug-Modus (Relay-Liste, Peers, Protokoll) |
| `#mpTurnBox` | TURN-Kasten der Lobby (M75): eigene Karte (`.panel`) mit Hinweis, Monospace-Textfeld (wie `#wsImportText`), 📋 und Speichern-Knopf plus Statuszeile. Erscheint NUR, wenn die Strecke gescheitert ist (`lobbyHint` = 'blocked') oder im Debug-Modus – ein Feld für Zugangsdaten gehört nicht in eine ruhige Lobby. Die Knopfzeile (`.ws-actions`) bricht auf dem Phone um, beide Knöpfe dehnen sich dann auf die ganze Breite |
...
| `.hud-chip.waiting` | Partner wartet im Ziel (M90, „gemeinsam ankommen"): Pille in `--partner` – derselben Farbe, in der er im Bild leuchtet. Sie steht NUR beim Nachzügler und verschwindet leer von selbst (`:empty`); der Wartende selbst liest seinen Zustand in `#status` und sieht sein Ziel ruhig leuchten |
